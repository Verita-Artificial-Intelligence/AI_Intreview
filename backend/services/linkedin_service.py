...
async def scrape_linkedin_profile(linkedin_url: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            print(f"Parsed data: firstName={parsed_data.get('firstName')}, lastName={parsed_data.get('lastName')}, expertise count={len(parsed_data.get('expertise', []))}")
...
def parse_linkedin_data(profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if sub_components and len(sub_components) > 0:
            ...
            if desc_list:
                ...
                description = " ".join([
                    ...
                    if isinstance(item, dict) and item.get("type") == "textComponent"
                ])
    ...
    address = profile.get("addressWithCountry", "") or profile.get("addressWithoutCountry", "")
    if address:
