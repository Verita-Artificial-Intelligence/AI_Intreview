...
async def migrate_statuses():
    ...
    result_open = await jobs_collection.update_many(
        {"status": "open"}, {"$set": {"status": "pending"}}
    )
    ...
    result_closed = await jobs_collection.update_many(
        {"status": "closed"}, {"$set": {"status": "archived"}}
    )
    ...
    print(
        f"Set 'pending' status for {result_null.modified_count} jobs with missing status"
    )

    # Show final status distribution
    all_jobs_after = await jobs_collection.find(
        {}, {"_id": 0, "id": 1, "title": 1, "status": 1}
    ).to_list(1000)
    status_counts_after = {}
