...
async def upload_video(
    ...
    try:
        if not interview_id or not session_id:
            raise HTTPException(
                status_code=400, detail="Interview ID and Session ID are required."
            )
        ...
        logger.error(
            f"Error uploading video for interview {interview_id}: {e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail="Failed to upload video.")


@router.post("/files/upload")
async def upload_file(file: UploadFile = File(...), job_id: str = Form(default=None)):
    """
    ...
    try:
        ...
        logger.error(
            f"Error uploading file {file.filename if file else 'unknown'}: {e}",
            exc_info=True,
        )
