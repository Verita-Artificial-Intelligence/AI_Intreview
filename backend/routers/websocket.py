    ...
    try:
        while True:
            ...
            if event_type == "mic_chunk":
                ...
                speech_density = (
                    recent_speech_count / len(recent_speech_chunks)
                    if recent_speech_chunks
                    else 0
                )
                ...
                if (
                    speech_density > 0.7
                ):  # Very high speech activity (70%+ of recent chunks)
                    ...
                    try:
                        ...
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug(
                                f"🗣️  Very high speech activity ({speech_density:.1%}) - extended silence window to {target_silence}ms"
                            )
                    except Exception as e:
                        logger.debug(
                            f"Failed to extend silence window during active speech: {e}"
                        )
                    ...
                    try:
                        ...
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug(
                                f"🎯 Moderate speech activity ({speech_density:.1%}) - using default silence window {target_silence}ms"
                            )
                    except Exception as e:
                    ...
                    target_silence = max(
                        2200, settings.OPENAI_REALTIME_SILENCE_DURATION_MS - 800
                    )
                    try:
                        await realtime.update_turn_detection(target_silence)
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug(
                                f"🤫 Low speech activity ({speech_density:.1%}) - reduced silence window to {target_silence}ms"
                            )
                    except Exception as e:
                        logger.debug(
                            f"Failed to reduce silence window during low activity: {e}"
                        )
