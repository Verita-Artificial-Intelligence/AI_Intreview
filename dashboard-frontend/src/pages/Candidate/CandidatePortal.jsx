...
export default function CandidatePortal() {
  ...
  const completedTasks = annotationTasks.filter(
                  ...
                  {pendingTasks.length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-neutral-700 mb-2">
                        Ready to Start ({pendingTasks.length})
                      </h3>
                      {pendingTasks.slice(0, 3).map((task) => (
                        <div
                          key={task.id}
                          className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 bg-neutral-50 rounded-lg mb-2"
                        >
                          ...
                          <Button
                            size="sm"
                            onClick={() =>
                              navigate(`/candidate/annotate/${task.id}`)
                            }
                  ...
                  {activeTasks.length > 0 && (
                    <div className="mt-4">
                      ...
                      {activeTasks.slice(0, 3).map((task) => (
                        <div
                          key={task.id}
                          className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 bg-yellow-50 rounded-lg mb-2"
                        >
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-neutral-900 truncate">
                              Task {task.id.slice(0, 8)}...
              ...
              {loading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-neutral-500 mt-1">
                    Apply for jobs to get started with interviews
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm text-neutral-600">
                            <span className="flex items-center gap-1">
                              ...
                              {new Date(
                                interview.scheduled_time || interview.created_at
                              ).toLocaleDateString()}
                            </span>
                            <span className="flex items-center gap-1">
                        ...
                        {interview.status === 'scheduled' && (
                          <Button
                            onClick={() =>
                              navigate(`/interview-prep/${interview.id}`)
                            }
