        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-neutral-600 uppercase font-semibold mb-1">Task ID</p>
              ...
              <div>
                ...
                <p className="text-sm">
                  {task.started_at ? new Date(task.started_at).toLocaleDateString() : 'Just now'}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              {/* Text Content */}
              {task.data_to_annotate?.data_type === 'text' && task.data_to_annotate?.data_content?.text && (
                <div>
                  <h4 className="font-semibold text-neutral-900 mb-3">Text Content</h4>
              ...
              {/* Image */}
              {task.data_to_annotate?.data_type === 'image' && task.data_to_annotate?.data_url && (
                <div>
                  <h4 className="font-semibold text-neutral-900 mb-3">Image</h4>
              ...
              {/* Video */}
              {task.data_to_annotate?.data_type === 'video' && task.data_to_annotate?.data_url && (
                <div>
                  <h4 className="font-semibold text-neutral-900 mb-3">Video</h4>
                  <div className="bg-neutral-50 p-4 rounded-lg border border-neutral-200">
                    <video
                      src={task.data_to_annotate.data_url}
                      controls
              ...
              {/* Audio */}
              {task.data_to_annotate?.data_type === 'audio' && task.data_to_annotate?.data_url && (
                <div>
                  <h4 className="font-semibold text-neutral-900 mb-3">Audio</h4>
                  <div className="bg-neutral-50 p-4 rounded-lg border border-neutral-200">
                    <audio
                      src={task.data_to_annotate.data_url}
                      controls
              ...
              {/* Document */}
              {task.data_to_annotate?.data_type === 'document' && task.data_to_annotate?.data_url && (
                <div>
                  <h4 className="font-semibold text-neutral-900 mb-3">Document</h4>
                  <div className="bg-neutral-50 p-4 rounded-lg border border-neutral-200">
                    <a
              ...
              {!task.data_to_annotate?.data_type && task.data_to_annotate?.transcript && (
                <div>
                  <h4 className="font-semibold text-neutral-900 mb-3">Transcript</h4>
                  <div className="bg-neutral-50 p-4 rounded-lg border border-neutral-200 max-h-64 overflow-y-auto">
                    <p className="text-sm text-neutral-700 whitespace-pre-wrap leading-relaxed">
                      {Array.isArray(task.data_to_annotate.transcript)
                        ? task.data_to_annotate.transcript.map(t => `${t.speaker}: ${t.text}`).join('\n\n')
